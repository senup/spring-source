...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	public int getResolvedAutowireMode() {
		if (this.autowireMode == AUTOWIRE_AUTODETECT) {
			...
			for (Constructor<?> constructor : constructors) {
				//如果构造方法中存在参数为0的情况，就通过类型来自动状态
				if (constructor.getParameterCount() == 0) {
					return AUTOWIRE_BY_TYPE;
				}
			}
			//如果Bean的所有构造方法都含有参数，就通过AUTOWIRE_CONSTRUCTORAUTOWIRE_CONSTRUCTOR来自动装配
			return AUTOWIRE_CONSTRUCTOR;
		}
		else {
			//默认的装配模式AUTOWIRE_NO 不自动装配
			return this.autowireMode;
		}
	}
	...
	public void prepareMethodOverrides() throws BeanDefinitionValidationException {
		...
		if (hasMethodOverrides()) {
			//对每一个方法都去判断有没有重载方法，一般是针对look-up和replace标签，不过目前很少用了
			//前者用于在抽象类里面指定继承类的方法；后者用于实现methodReplace接口，然后标签指定是这个replace的方法
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		if (count == 0) {
			...
			//如果计算重载的地方只有一个，所以没有重载方法
			//此时，Spring就会提前通过代码mo.setOverloaded（talse），设置该方法不需要去重载方法中寻找，
			// 到时候要覆盖这个方法时，直接根据方法名称到bean中寻找即可，避免了在重载方法中寻找匹配方法的性能损耗。
